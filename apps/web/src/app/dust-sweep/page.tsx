...
export default function DustSweepPage() {
  ...
  useEffect(() => {
    function handleClickOutside(e: MouseEvent) {
      ...
      if (!target.closest('[data-output-dropdown]')) {
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 sm:py-12">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400 text-lg">
            Select your dust tokens and sweep them into one
          </p>
        </div>
          ...
          <div className="flex items-center justify-between mb-3">
            <p className="text-sm text-gray-400">
              {isLoading ? (
                ...
                <>
                  <span className="text-white font-medium">{selectedCount}</span> of{' '}
                  <span className="text-white font-medium">{MAX_SELECTED}</span> max selected
                </>
              )}
            </p>
            {dustTokens.length > 0 && (
              ...
              <>
                ...
                {dustTokens.map((token) => (
                  <TokenCard
                    key={token.address}
                    token={token}
                    isSelected={selectedTokens.some(
              ...
              {isQuoting ? (
                <div className="flex items-center justify-center py-6">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-gray-400 text-sm">Getting best quote...</span>
                  </div>
                </div>
              ) : quoteError ? (
                <div className="text-center py-4">
                  <p className="text-red-400 text-sm mb-2">{quoteError}</p>
                  <button
                    onClick={getQuote}
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-gray-400">Swap fee ({quote.swapFeePercent}%)</span>
        ...
        {/* ── Action Button ──────────────────────────────────────────────── */}
        {hasSelectedTokens && quote && sweepCalls.length > 0 && (
          <div className="mb-8">
            <Transaction
              chainId={8453}
              calls={sweepCalls}
            ...
            >
              <TransactionButton
                ...
                className="w-full !bg-gradient-to-r !from-purple-600 !to-indigo-600 hover:!from-purple-500 hover:!to-indigo-500 !text-white !font-semibold !py-4 !rounded-xl !text-base !shadow-lg !shadow-purple-500/25 !transition-all !duration-200"
              />
              <TransactionStatus>
                <TransactionStatusLabel className="text-sm text-gray-400 mt-2" />
                <TransactionStatusAction className="text-sm text-purple-400 mt-1" />
              </TransactionStatus>
            </Transaction>
          </div>
        )}

        {/* ── Disabled Action Button (no quote yet) ──────────────────────── */}
        {hasSelectedTokens && (!quote || sweepCalls.length === 0) && !isQuoting && (
          <div className="mb-8">
            <button
              onClick={getQuote}
              disabled={isQuoting}
              className="w-full py-4 rounded-xl bg-gray-800 text-gray-500 font-semibold cursor-not-allowed"
            >
              {quoteError ? 'Retry Quote' : 'Waiting for quote...'}
            </button>
          </div>
        )}
        ...
        <div className="text-center">
          <p className="text-xs text-gray-600">
            DustSwap charges a 2% fee on swaps. Gas is sponsored via Base Paymaster.
          </p>
          <p className="text-xs text-gray-600 mt-1">
            Earn ✨ Dust Particles with every sweep!
          </p>
        </div>
      </div>
    </div>
