...
// ─────────────────────────────────────────────────────────────────────────────
// DustSwapSweepRouterArbitrum (DustSweep V3 — Arbitrum One) — flattened single-file build for
// Remix. Arbitrum One (chainId 42161) adaptation of DustSwapSweepRouter_Remix.sol (which is kept
// unchanged). Identical logic and external ABI; only the contract name differs.
// Arbitrum's native gas token IS ETH and its wrapped native is literally WETH
// (0x82aF49447D8a07e3bd95BD0d56f35241523fBab1, verified live 2026-08-07: symbol()=WETH,
// decimals()=18), so — unlike the BSC build's wbnb/IWBNB rename — the weth/IWETH9 naming and
// every "native ETH" comment stay verbatim: they are factually correct on this chain. This
// follows the Ethereum/Robinhood precedent, not BSC's.
//
// REMIX / Arbiscan VERIFY SETTINGS (use these EXACTLY for first-try verification):
//   Compiler:    0.8.24+commit.e11b9ed9   (Remix: "0.8.24")
//   Language:    Solidity
//   EVM version: cancun                   (set explicitly; do NOT leave "default")
//                                         ArbOS 106 is live — far past the ArbOS 40 Pectra floor.
...
// src/DustSwapSweepRouter.sol (Arbitrum One adaptation — contract name only; weth/IWETH9 kept)
...
/// @title DustSwapSweepRouterArbitrum (DustSweep V3 — Arbitrum One)
/// @author DustSwap
/// @notice Best-effort, multi-DEX dust sweep router for Arbitrum One (chainId 42161). Sweeps
...
/// @dev    Arbitrum One deployment of the V3 contract — same logic and external ABI as the
///         Base/Ethereum/BSC/Robinhood builds; the app targets it via
///         DUST_SWEEP_ROUTER_V3_ADDRESS_42161.
...
contract DustSwapSweepRouterArbitrum is Ownable, ReentrancyGuard, Pausable {
    ...
    /// @param permit2_      Canonical Permit2 on Arbitrum One (same address on every chain,
    ///                      verified live 2026-08-07): 0x000000000022D473030F116dDEE9F6B43aC78BA3
    /// @param weth_         WETH on Arbitrum One: 0x82aF49447D8a07e3bd95BD0d56f35241523fBab1
